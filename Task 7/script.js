// Dummy data (trimmed for brevity)
const data = {
  "posts": [
    {
      "id": 1,
...
data.posts.forEach(post => {
  ...
  postDiv.style.backgroundColor = post.views > 2000 ? "#f0f8ff" : "#ffe4e1";

  // Title
  let title = document.createElement('h2');
  title.textContent = post.title;
  title.style.color = post.reactions.likes > 500 ? "green" : "red";
  ...
  const colors = ["#ff9999", "#99ff99", "#9999ff", "#ffcc99", "#66ccff"];

  post.tags.forEach((tag, index) => {
    let tagBtn = document.createElement('button');
    tagBtn.textContent = tag;
    tagBtn.style.margin = "5px";
    tagBtn.style.padding = "5px 10px";
    tagBtn.style.border = "none";
    tagBtn.style.borderRadius = "5px";
    tagBtn.style.cursor = "pointer";
    tagBtn.style.backgroundColor = colors[index % colors.length];
